//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSArray;
@protocol OS_dispatch_data;

__attribute__((visibility("hidden")))
@interface MTLBinaryEntry : NSObject
{
    NSObject<OS_dispatch_data> *_data;	// 8 = 0x8
    NSArray *_importedLibraries;	// 16 = 0x10
    NSArray *_importedSymbols;	// 24 = 0x18
    NSObject<OS_dispatch_data> *_bitcode;	// 32 = 0x20
    NSObject<OS_dispatch_data> *_airScript;	// 40 = 0x28
    NSObject<OS_dispatch_data> *_reflectionBlock;	// 48 = 0x30
    int _reflectionFlags;	// 56 = 0x38
    unsigned long long _binaryPosition;	// 64 = 0x40
    unsigned long long _index;	// 72 = 0x48
}

@property unsigned long long index; // @synthesize index=_index;
@property(readonly) NSArray *importedLibraries; // @synthesize importedLibraries=_importedLibraries;
@property(readonly) NSArray *importedSymbols; // @synthesize importedSymbols=_importedSymbols;
@property(readonly) int reflectionFlags; // @synthesize reflectionFlags=_reflectionFlags;
@property(readonly) unsigned long long binaryPosition; // @synthesize binaryPosition=_binaryPosition;
@property NSObject<OS_dispatch_data> *airScript; // @synthesize airScript=_airScript;
@property NSObject<OS_dispatch_data> *bitcode; // @synthesize bitcode=_bitcode;
- (id)description;	// IMP=0x0000000000047a53
@property(readonly) NSObject<OS_dispatch_data> *reflectionBlock; // @synthesize reflectionBlock=_reflectionBlock;
@property(readonly) NSObject<OS_dispatch_data> *data; // @synthesize data=_data;
- (void)dealloc;	// IMP=0x00000000000479c2
- (id)initWithData:(id)arg1;	// IMP=0x00000000000479ab
- (void)addReflectionWithData:(id)arg1 flag:(int)arg2;	// IMP=0x0000000000047979
- (id)initWithData:(id)arg1 reflectionBlock:(id)arg2;	// IMP=0x0000000000047913
- (id)initWithData:(id)arg1 reflectionBlock:(id)arg2 binaryPosition:(unsigned long long)arg3;	// IMP=0x00000000000478a9
- (void)internalInitWithData:(id)arg1 reflectionBlock:(id)arg2 binaryPosition:(unsigned long long)arg3;	// IMP=0x000000000004784b
- (id)initWithData:(id)arg1 binaryPosition:(unsigned long long)arg2;	// IMP=0x00000000000477c3
- (id)initWithData:(id)arg1 importedSymbols:(id)arg2 importedLibraries:(id)arg3;	// IMP=0x0000000000047721

@end

