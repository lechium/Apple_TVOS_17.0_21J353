//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import "VUIJSObject.h"

@protocol VUIAppUserDefaultsStoring;

__attribute__((visibility("hidden")))
@interface VUIJSUserDefaults : VUIJSObject
{
    id <VUIAppUserDefaultsStoring> _userDefaultsStorage;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x0000000000175b4a
@property(nonatomic) __weak id <VUIAppUserDefaultsStoring> userDefaultsStorage; // @synthesize userDefaultsStorage=_userDefaultsStorage;
- (void)removeData:(id)arg1;	// IMP=0x0000000000175ab4
- (void)setData:(id)arg1:(id)arg2;	// IMP=0x0000000000175908
- (id)getData:(id)arg1;	// IMP=0x00000000001757cc
- (id)initWithAppContext:(id)arg1 userDefaultsStorage:(id)arg2;	// IMP=0x000000000017575b

@end

