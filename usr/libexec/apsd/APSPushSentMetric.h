//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSDictionary, NSNumber, NSString, PBCodable;
@protocol NSCopying;

@interface APSPushSentMetric : NSObject
{
    NSString *_guid;	// 8 = 0x8
    unsigned int _connectionType;	// 16 = 0x10
    NSNumber *_linkQuality;	// 24 = 0x18
    unsigned int _dualChannelState;	// 32 = 0x20
    NSNumber *_flushes;	// 40 = 0x28
    NSNumber *_sendDuration;	// 48 = 0x30
    NSNumber *_payloadSize;	// 56 = 0x38
    unsigned int _error;	// 64 = 0x40
    NSString *_topic;	// 72 = 0x48
}

- (void).cxx_destruct;	// IMP=0x00200000000404d3
@property(readonly, nonatomic) NSString *topic; // @synthesize topic=_topic;
@property(readonly, nonatomic) unsigned int error; // @synthesize error=_error;
@property(readonly, nonatomic) NSNumber *payloadSize; // @synthesize payloadSize=_payloadSize;
@property(readonly, nonatomic) NSNumber *sendDuration; // @synthesize sendDuration=_sendDuration;
@property(readonly, nonatomic) NSNumber *flushes; // @synthesize flushes=_flushes;
@property(readonly, nonatomic) unsigned int dualChannelState; // @synthesize dualChannelState=_dualChannelState;
@property(readonly, nonatomic) NSNumber *linkQuality; // @synthesize linkQuality=_linkQuality;
@property(readonly, nonatomic) unsigned int connectionType; // @synthesize connectionType=_connectionType;
@property(readonly, nonatomic) NSString *guid; // @synthesize guid=_guid;
@property(readonly, nonatomic) unsigned short rtcType;
@property(readonly, nonatomic) PBCodable<NSCopying> *awdRepresentation;
@property(readonly) unsigned int awdIdentifier;
- (id);	// IMP=0x00100000000401b3
@property(readonly, nonatomic) NSString *name;
- (id)initWithGuid:(id)arg1 connectionType:(unsigned int)arg2 linkQuality:(id)arg3 dualChannelState:(unsigned int)arg4 flushes:(id)arg5 sendDuration:(id)arg6 payloadSize:(id)arg7 error:(unsigned int)arg8 topic:(id)arg9;	// IMP=0x0010000000040034

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly, nonatomic) NSDictionary *dictionaryRepresentation;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

